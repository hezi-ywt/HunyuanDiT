...
def filter_by_size_ratio(img_pth, ratio=3):
...
def filter_and_move(img_pth, trash_dir="/home1/qbs/my_program1/datahun2/trash", ratio=4):
    txt_pth = os.path.splitext(img_pth)[0] + ".txt"
    if not os.path.exists(trash_dir):
        os.makedirs(trash_dir)
...
def filter_dir(img_dir):
    files = [os.path.join(img_dir, file) for file in os.listdir(img_dir) if any(file.endswith(end) for end in IMG_ENDWITH)]
    with Pool(processes=cpu_count()) as pool:
        for _ in tqdm(pool.imap_unordered(filter_and_move, files), total=len(files), desc='Processing'):
            pass

def main():
    img_dir = "/home1/qbs/my_program1/datahun2/artist"
    out_dir = "/home1/qbs/my_program1/datahun2/trash"
